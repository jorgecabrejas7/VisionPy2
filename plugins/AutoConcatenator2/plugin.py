...
class Plugin(BasePlugin):
    def run(self):
        ...
        save_path = self.select_folder("Select Folder to save concatenated volume")
    ...
    def get_info(self, vol1shape, vol2shape, show=False):
        ...
        search_range, ok = QInputDialog.getInt(
            self.main_window, "Search Range", "Enter the slices search range:"
        )
        ...
        for name in names:
            while True:
                ...
                if top_threshold != None:
                    ...
                    largest1 = regprops1[
                        np.argmax([regprop.area for regprop in regprops1])
                    ].label
                    largest2 = regprops2[
                        np.argmax([regprop.area for regprop in regprops2])
                    ].label
    ...
    def compare_slices_concurrent_ai(
        ...
        def pad_and_crop_image(image, square_size):
            """
            Pad an image with zeros and crop it into squares.

            Args:
            image (numpy.ndarray): The image to be padded and cropped.
            square_size (int): The size of the squares to crop the image into.

            Returns:
            numpy.ndarray: A 3D numpy array where each slice is a cropped square.
            """
            ...
            cropped_images = []
            for i in range(0, resized_image.shape[0], square_size):
                for j in range(0, resized_image.shape[1], square_size):
                    cropped_images.append(
                        resized_image[i : i + square_size, j : j + square_size]
                    )
        ...
        def prepare_images_for_model(img1):
            # patch it
            imgs = pad_and_crop_image(img1, 224)
        ...
        indexes = self.find_candidates_volume(
        ...
        args = [
            (features1[i - start - 1], features2[j - first - 1], i, j)
            for i, j in indexes
        ]
    ...
    def find_candidates_volume(
        self, vol1, vol2, length, resolution, indexes, range=200
    ):
        candidates = []

        for i in indexes:
            candidates.append(
                self.find_candidates_slice(
                    i,
                    length,
                    resolution,
                    vol1.shape[0],
