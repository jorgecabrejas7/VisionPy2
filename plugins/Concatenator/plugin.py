...
class Plugin(BasePlugin):
    def run(self):
        ...
        if file_path and file_path2:
            ...
            i, j = self.request_gui(
                self.get_candidates, volume1.shape[0], volume2.shape[0], show=False
            )
            ...
            if len(user_inputs) > 0:
                ...
                # shift volume
                volume2 = self.shift_volume_concurrent(
                    volume2, shift, progress_window=self
                )
    ...
    def get_candidates(self, vol1shape, vol2shape, show=False):
        ...
        while True:
            frameid, ok = QInputDialog.getInt(
                self.main_window,
                "Frameid",
                "Frameid of slice to search candidates \nFrom "
                + str(slice1)
                + " to "
                + str(vol1shape - 1)
                + ":",
        ...
        # find candidates
        candidates = self.find_candidates_slice(
            frameid, length, resolution, vol2shape, show=True
        )
        ...
        while True:
            frameid, ok = QInputDialog.getInt(
                self.main_window,
                "Frameid",
                "Enter the frame of the first volume to concatenate:",
            )
            frameid2, ok = QInputDialog.getInt(
                self.main_window,
                "Frameid2",
                "Enter the frame of the second volume to concatenate:",
            )

            if (
                frameid >= slice1
                and frameid < vol1shape
                and frameid2 >= 0
                and frameid2 < candidates[-1]
            ):
                break
    ...
    def find_candidates_slice(
        self, frameid, length, resolution, vol2shape, range_slices=200, show=False
    ):
        """
