...
class Plugin(BasePlugin):
    ...
    def run(self):
        ...
        save_path = self.select_folder(
            "Select Folder to save centered volume"
        )
        ...
        if file_path:
            ...
            if len(user_inputs) > 0:
                ...
                #shift volume
                volume = self.shift_volume_concurrent(volume,shift,progress_window=self)
                ...
                if user_inputs2:
                    ...
                    volume = self.crop_volume(volume,user_inputs2['Main'][0],user_inputs2['Main'][1])
    ...
    def get_dimensions(self,volume):
        ...
        for name in names:
            ...
            while True:
                ...
                dimensions = self.get_user_inputs2(name, default_values=[volume.shape[1],volume.shape[2]])
    ...
    def get_user_inputs2(self,name,default_values=[0,0]):
        #Get the threshold value for the name reslice
        x, ok = QInputDialog.getInt(self.main_window, "X", f"Enter X for {name} reslice:", default_values[0])
        y, ok = QInputDialog.getInt(self.main_window, "Y", f"Enter Y for {name} reslice:", default_values[1])
