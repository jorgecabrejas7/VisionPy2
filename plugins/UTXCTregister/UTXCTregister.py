...
def register(labeled_ut, labeled_xct, save_path):
    ...
    registration_method.SetOptimizerAsExhaustive(
        numberOfSteps=[5, 5, 5], stepLength=1.0
    )
...
def apply_transform_parameters(
    matrix, ut, xct, original_resolution=0.025
):  # (X,Y,Z) axes
...
def main(ut, rf, xct, padded=False):
    if not padded:
        ...
        xct = np.pad(
            xct,
            (
                (pad_diff[0] // 2, pad_diff[0] - pad_diff[0] // 2),
                (pad_diff[1] // 2, pad_diff[1] - pad_diff[1] // 2),
                (0, 0),
            ),
            mode="constant",
            constant_values=0,
    ...
    transformation_matrix = rigid_body_transformation_matrix(
        sorted_ut_points[:, :2], sorted_xct_points[:, :2]
    )
    ...
    for i in range(rf.shape[2]):
        transformed_volume.append(
            scipy.ndimage.affine_transform(
                ...
                output_shape=xct_centers.shape,
            )
        )
...
def main_2(ut, xct):
    ...
    transformed_xct_centers = apply_transform_parameters(
        transformation_matrix, ut, (xct_centers > 0) * 255, original_resolution=1
    )
