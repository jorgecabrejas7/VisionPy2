...
class Plugin(BasePlugin):
    def run(self):
        ...
        save_path = self.select_folder(
            "Select Folder to save registered volume"
        )
        ...
        if file_path and file_path2:
            ...
            start, end = self.request_gui(self.get_info, bot_roi.shape[0],top_roi.shape[0],show=False)
            ...
            if len([1]) > 0:
                ...
                mask_top_full = self.mask(sitk.GetArrayFromImage(registered_volume_top_overlapping))
                ...
                if save_path:
                    ...
                    image_sequence.write_sequence2(save_path, os.path.basename(save_path), sitk.GetArrayFromImage(registered_volume_top),progress_window=self)
