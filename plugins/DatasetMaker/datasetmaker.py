...
def crop_image_to_patch_size(image, patch_size):
    ...
    cropped_image = image[
        :, crop_x_before : -crop_x_after or None, crop_y_before : -crop_y_after or None
    ]
...
def patch(
    ...
    ut_step_size = 1
    xct_patch_size = int(
        np.round(calculate_pixels(ut_resolution, xct_resolution, ut_patch_size))
    )
    xct_step_size = int(
        np.round(calculate_pixels(ut_resolution, xct_resolution, ut_step_size))
    )
    ...
    xct_shape = calculate_patch_shape(
        onlypores_cropped.shape, xct_patch_size, xct_step_size
    )
    ...
    patches_onlypores = divide_into_patches(
        onlypores_cropped, xct_patch_size, xct_step_size
    )
    center_size = int(patches_onlypores.shape[2] / ut_patch_size)
    patches_onlypores = patches_onlypores[
        :, :, center_size:-center_size, center_size:-center_size
    ]
...
def clean_pores_3D(patches_onlypores):
    ...
    def process_slice(i, patches_onlypores):
        dilated_image = scipy.ndimage.binary_dilation(
            patches_onlypores[i], iterations=2
        )  # dilate to group near pores
    ...
    results = Parallel(n_jobs=-1, backend="loky")(
        ...
        for i in range(patches_onlypores.shape[0])
    )
...
def datasets1(patches_onlypores, patches_mask, patches_ut, folder, ut_patch_size):
    ...
    patch_vs_volfrac = np.hstack(
        (ut_patches_reshaped, volfrac.reshape(-1, 1), areafrac.reshape(-1, 1))
    )

    df_patch_vs_volfrac = pd.DataFrame(
        patch_vs_volfrac, columns=np.append(columns_ut, ["volfrac", "areafrac"])
    )

    # save the dataframes

    df_patch_vs_volfrac.to_csv(
        folder / ("patch_vs_volfrac_" + str(ut_patch_size) + ".csv"), index=False
    )
...
def main(
    ...
    # patch the images
    patches_onlypores, patches_mask, patches_ut = patch(
        onlypores_cropped,
        mask_cropped,
        ut_rf_cropped,
        ut_patch_size,
